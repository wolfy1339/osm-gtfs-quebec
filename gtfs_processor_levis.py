...
master_route_tmpl = """
    (
    relation["type"="route_master"]["route_master"="bus"]["operator"="Société de transport de Lévis"];
    );
    out body;
"""
...
area({})->.searchArea;
    (
    node["highway"="bus_stop"](area.searchArea);
    relation["type"="route"]["route"="bus"]["network"="STLévis"](area.searchArea);
    relation["type"="route"]["route"="bus"]["network"="Société de transport de Lévis"](area.searchArea);
    relation["type"="master_route"]["route_master"="bus"]["operator"="Société de transport de Lévis"];
    );
    out body;
"""
...
class GTFSProcessor():
    ...
    def __init__(self, gtfs_zipfile: str, boundaries_dir: str, output_dir: str):
        ...
        self.gtfs_dir = os.path.join(os.path.dirname(self.gtfs_zipfile), 'gtfs-levis')
    ...
    def get_latest_service_id(self):
        ...
        for service in self.gtfs_data['calendar_dates']['data']:
            if parse(service['date']).weekday() < 5:
                dates.add(service['date'])

        max_date = max(dates)

        self.service_prefixes = set[str]()
        for service in self.gtfs_data['calendar_dates']['data']:
            if service['date'] == max_date:
                self.service_prefixes.add(service['service_id'])
                logger.info('Latest service is %s', service['service_id'])

    def filter_gtfs_data(self):
        logger.info('Filtering the GTFS data...')
        prefixes = self.service_prefixes

        filtered_trips = [x for x in self.gtfs_data['trips']['data'] for p in prefixes if x['service_id'] == p]
        ...
        for stop in tqdm(self.gtfs_data['stops']['data']):
            ...
            gtfs_stop: GTFSStop = {
                ...
                'tags': {
                    ...
                    'ref': stop['stop_code'],
                    'network:wikidata': 'Q3488027',
                    'network': 'STLévis',
                    'operator': 'Société de transport de Lévis',
    ...
    def get_existing_osm_data(self):
        ...
        stops_result_levis = api.query(bus_stop_tmpl.format(region_ids['Levis'] + relation_to_area_factor))
        stops_result_st_lambert = api.query(bus_stop_tmpl.format(region_ids['St-Lambert-de-Lauzon'] + relation_to_area_factor))
        time.sleep(30)

        for node in tqdm(stops_result_levis.nodes+stops_result_st_lambert.nodes):
        ...
        routes_result_levis = api.query(service_route_tmpl.format(region_ids['Levis'] + relation_to_area_factor))
        route_masters_result_levis = api.query(master_route_tmpl)

        values = ['STLévis', 'Société de transport de Lévis']

        for relation in routes_result_levis.get_relations():
        ...
        for relation in route_masters_result_levis.get_relations():
            tags: dict[str, str] = relation.tags # type: ignore
            true_count = 0
        ...
        logger.info('Found %s existing stops Lévis', len(existing_stops))
        logger.info('Found %s existing routes Lévis', len(existing_routes))
        logger.info('Found %d existing master routes Lévis', len(existing_route_masters))
    ...
    def conflate_stops(self):
        ...
        logger.info('Filtering coverages for Levis')
        coverage_osm_levis = [osm_buffer for osm_buffer in iter(coverage_osm) if osm_buffer.Intersects(self.boundaries['Levis'])]
        ...
        for gtfs_buffer in tqdm(coverage_gtfs):
            ...
            for osm_buffer in coverage_osm_levis:
    ...
    def create_relations(self):
        ...
        for key, route_master in tqdm(route_masters.items()):
            ...
            for route in route_master:
                ...
                for trip in trips:
                    ...
                    route_relation: RouteRelation = {
                        ...
                        "tags": {
                            "name": f"Parcours {route_ref} vers {last_stop_name}" if not route_ref in ["L1","L2","L3"] else f"Lévisien {route_ref[1:]} vers {last_stop_name}",
                            "ref": route_ref,
                            "type": "route",
                            "route": "bus",
                            "network": "STLévis",
            ...
            route_master_relation: RouteMasterRelation = {
                ...
                "tags": {
                    "name": f"Parcours {key}" if not key in ["L1","L2","L3"] else f"Lévisien {key[1:]}",
                    "ref": key,
                    "network": "STLévis",
                    "network:wikidata": "Q3488027",
                    "operator": "Société de transport de Lévis",
    ...
    def conflate_relations(self):
        ...
        for route_relation in self.route_relations:
            ...
            trip_id = route_relation['tags']['gtfs:trip_id']
            headboard = next(trip for trip in self.gtfs_data['trips']['data'] if trip['trip_id'] == trip_id)['trip_headsign']

            existing_routes = existing_routes_by_ref.get(new_ref, [])

            # Find a matching existing route
            existing_route = next((r for r in existing_routes if r['tags'].get('name') == new_name or headboard in r['tags'].get('name','')), None)
            if existing_route is not None:
                ...
                route_relation['tags']['colour'] = existing_route['tags'].get('colour') or route_relation['tags']['colour']
        ...
        for route_master in self.route_master_relations:
            ...
            if existing_master:
                ...
                route_master['tags']['colour'] = existing_master['tags'].get('colour', '')

    def write_to_xml(self, types: list[str]):
        #print(types)
        logger.info('Writing JOSM XML files for %s.', ', '.join(types))
        output_file = os.path.join(self.output_dir, 'gtfs_levis.xml')
    ...
    def get_route_stops(self, route_id: str):
        ...
        for trip in self.gtfs_data['trips']['data']:
            if trip['route_id'] == route_id:
                ...
                trips[trip_id]['trip_headsign'] = trip['trip_headsign']
...
cwd = os.getcwd()
gtfs_zipfile_location = os.path.join(cwd, 'gtfs_stlevis.zip')
boundaries_dir = os.path.join(cwd, 'boundaries')
output_dir = os.path.join(cwd, 'output-levis')

time1 = time.time()
gtfs_processor = GTFSProcessor(gtfs_zipfile_location, boundaries_dir, output_dir)
...
gtfs_processor.write_to_xml(['routes','stops','route_masters'])
